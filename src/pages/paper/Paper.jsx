...
// const mainData = [
//   { course: "MCA", align: "right", semesters: 4, details: "CLICK HERE TO GET PREVIOUS MCA PAPER", color: "skyBlue" },
//   { course: "MBA", align: "left", semesters: 4, details: "CLICK HERE TO GET PREVIOUS MBA PAPER", color: "pink" },
//   { course: "BCA", align: "right", semesters: 6, details: "CLICK HERE TO GET PREVIOUS BCA PAPER", color: "skyBlue" },
//   { course: "BBA", align: "left", semesters: 6, details: "CLICK HERE TO GET PREVIOUS BBA PAPER", color: "pink" },
// ];
...
//                 //     return { active: true, reverse: false };
//                 //   } else if (!entry.isIntersecting && state.active) {
//                 //     return { active: false, reverse: true };
//                 //   }
//                 // }
...
//     navigate(`/papers/${courseLower}/${sem}`);
//   };
...
//           Scroll to explore MCA, MBA, BCA, and BBA timelines.
//         </p>
...
export const Paper = () => {
  ...
  // ✅ ADDED: Include `locked` to manage manual toggle
  const [activeStates, setActiveStates] = useState(
    mainData.map(() => ({ active: false, reverse: false, locked: false }))
  );
  ...
  // ✅ HANDLER: Click on detail card toggles lock + active state
  const handleCourseClick = (index) => {
    setActiveStates((prevStates) =>
      prevStates.map((state, i) =>
        i === index
          ? { active: !state.active, reverse: state.active, locked: !state.locked }
          : state
      )
    );
  };
  ...
  return (
    <div className="main-container">
      <section className="hero-section" ref={arrowStartRef}>
        <img className="ball" src={Ball} alt="Ball" />
        <img className="book-top" src={BookTop} alt="Top Book" />
        <h1 className={`hero-heading ${animate ? "animate-heading" : ""}`}>Previous Year Papers</h1>
        <p className={`hero-subtext ${animate ? "animate-subtext" : ""}`}>
          Scroll to explore MCA, MBA, BCA, and BBA timelines.
        </p>
        <div className="scroll-indicator">Scroll Down ↓</div>
      ...
      <section className="timeline-section">
        {mainData.map((item, index) => {
          const { active, reverse, locked } = activeStates[index];
          ...
          return (
            <div
              ...
              data-index={index}
            >
              {/* ✅ Click on left detail card toggles orbit + lock */}
              {item.align === "left" && (
                <div className={`details-card ${item.color}`} onClick={() => handleCourseClick(index)}>
                  {item.details}
                </div>
              )}
              ...
              {/* ✅ Hover only works if not locked; click toggles lock */}
              <div
                className="circle"
                onMouseEnter={() => {
                  if (!locked) {
                    setActiveStates((prev) =>
                      prev.map((s, i) =>
                        i === index ? { ...s, active: true, reverse: false } : s
                      )
                    );
                  }
                }}
                onMouseLeave={() => {
                  if (!locked) {
                    setActiveStates((prev) =>
                      prev.map((s, i) =>
                        i === index ? { ...s, active: false, reverse: true } : s
                      )
                    );
                  }
                }}
                onClick={() => {
                  setActiveStates((prev) =>
                    prev.map((s, i) =>
                      i === index
                        ? { ...s, active: !s.active, reverse: s.active, locked: !s.locked }
                        : s
                    )
                  );
                }}
              ...
              {/* ✅ Click on right detail card toggles orbit + lock */}
              {item.align === "right" && (
                <div className={`details-card ${item.color}`} onClick={() => handleCourseClick(index)}>
                  {item.details}
                </div>
